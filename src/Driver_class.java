                        ...
                        try {
                            ...
                            while (!A.isEmpty()) {
                                ...
                                if (!A.isEmpty()) {
                                    ...
                                    p1.add(process);
                                    p1.add(process1);
                                    if (process.getPriority() < process1.getPriority()) {
                                    ...
                                    } else if (process1.getPriority() > process.getPriority()) {
                                        ...
                                        if (process.getPriority() < process1.getPriority()) {
                                            ...
                                            if (process.getArrival_time() > process1.getArrival_time()) {//
                                                while (process.getArrival_time() < process1.getArrival_time()) {
                                                    int a = process.getArrival_time();
                                                    process.setArrival_time(a--);
                                                    System.out.print("P" + process1.getProcessID() + " | ");
                                                    fileWriter.write("P" + process1.getProcessID() + " | ");
                                                }
                                            }
                            ...
                            while (!p1.isEmpty()) {
                                ...
                                int t = process.getArrival_time();
                                process.setTermination_time(time + t);
                                int ter = process.getTermination_time();
                                int st = process.getStart_Time();
                                process.setResponse_time(st - t);
                                int c = process.getCPU_burst();
