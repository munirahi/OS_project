...
public class Driver_class {
    ...
    public static  Queue<PCB> RR_not_done= new LinkedList<>();
...
//int count=0;
                    ...
                    ((List<PCB>) q2_temp).sort(Comparator.comparingInt(PCB::getCPU_burst));
                    ((List<PCB>) q2_temp).sort(Comparator.comparingInt(PCB::getArrival_time));
                    readyQ();
//                    while (!readyQForAll.isEmpty()){
//                        System.out.print(readyQForAll.poll().ProcessID + "|" );
//                    }
...
//                    readyQ(q1,q2_temp);
    ...
    public static void readyQueue(){
        PCB P_from1InReady=null;
        PCB P_from2InReady=null;
        while(!q1.isEmpty() || !q2_temp.isEmpty()){
            if(!q1.isEmpty()){

                P_from1InReady = (PCB) q1.poll();
            }
            if(!q2_temp.isEmpty()){

                P_from2InReady = (PCB) q2_temp.poll();
            }
            if (P_from1InReady.getArrival_time() <= current_time){
                RR_calc(P_from1InReady);

            }else if( P_from2InReady.getArrival_time() <= current_time){
                SJF_calc(P_from2InReady);
    ...
    public static void readyQ(){
        PCB P_from1InReady=null;
        PCB P_from2InReady=null;
        String s = "[" ;
        if(!q1.isEmpty() || !q2_temp.isEmpty() ) {
            while (!q1.isEmpty() || !q2_temp.isEmpty()){
                if(!q1.isEmpty()){
                ...
                try {
                    if (P_from1InReady.getArrival_time() <= P_from2InReady.getArrival_time() ) {
                        ...
                        addToBeginning(q2_temp, P_from2InReady);
                        s += P_from1InReady.ProcessID + "| ";
...
//                    }
                        ...
                        SJF_calc(P_from2InReady);
                        addToBeginning(q1, P_from1InReady);
                        s += P_from2InReady.ProcessID + "|";
...
//                } not needed
            ...
            System.out.println("Q1 InReady is Empty and Q2 InReady is Empty");
        }
    ...
    public static void RR_calc(PCB p){
    ...
    public static void SJF_calc(PCB process){
    ...
    public static void SJF(Queue processes){
        ...
        Queue<PCB> readyQueue = new LinkedList<>(processes);
        ((List<PCB>) readyQueue).sort(Comparator.comparingInt(PCB::getCPU_burst));
        ((List<PCB>) readyQueue).sort(Comparator.comparingInt(PCB::getArrival_time));
